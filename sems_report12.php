...
if (isset($_POST['sprn'])) {
  ...
  $getData4 = mysqli_query($conn, "select * from `academic12` where `PRN`='$PRN'");
        ...
        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
          <tbody>
            <tr>
              ...
              <td><?php echo "First Year"; ?></td>
              <th><label for="sem">SEM</label></th>
              <td><?php echo "Second Semester"; ?></td>
            </tr>
          </tbody>
        </table> 
      </div> 
...
while ($row2 = mysqli_fetch_assoc($getAssignedStudents)) {
  ...
  // Check if all fields are not empty for the given PRN
  $isReportFilled = false; // Assume report is not filled initially
  $checkQuery = mysqli_query($conn, "SELECT * FROM semrepo12 WHERE PRN='$prn'");
  if ($checkQuery) {
    $reportData = mysqli_fetch_assoc($checkQuery);

    // Check if all fields are not empty
    if (!empty($reportData['Review']) &&
        !empty($reportData['Attendance']) &&
        !empty($reportData['Result']) &&
        !empty($reportData['Carricular']) &&
        !empty($reportData['CandP_Skills']) &&
        !empty($reportData['Behaviour']) &&
        !empty($reportData['Other']) &&
        !empty($reportData['TID'])) {
      $isReportFilled = true; // All fields are not empty, report is filled
    }
  }
  ...
  if (!$isReportFilled) {
    ...
    echo '<td>
            <select name="sprn" title="Select your PRN">
              <option value="' . $row2['PRN'] . '">' . $row2['Name'] . ' (' . $row2['PRN'] . ' - ' . $row2['Class'] . ')</option>
            </select>
          </td>';
  }
}
?>
      ...
      <div class="container-fluid">
        <div class="card shadow mb-4">
          <div class="card-header py-3" style="background-color: #4e73df;" data-toggle="collapse" href="#academicRecord11Table">
            <h6 class="m-0 font-weight-bold" style="color: #fff;">Academic Record 12</h6>
          </div>
  ...
  <tr>
    <th colspan="6"><h4>Academic Report (12)</h4> </th>
  </tr>
  ...
  <tr>
    ...
    <div class="card shadow mb-4">
      <div class="card-header py-3" style="background-color: #4e73df;">
        <h6 class="m-0 font-weight-bold" style="color: #fff;">Generated Report (First Year - Second Semester)</h6>
      </div>
      <div class="card-body">
        <div class="table-responsive text-center">
          <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
      ...
      <tr>
          ...
          <td><label for="review12"></label><?php echo $row['Review']; ?></td>
      </tr>
      <tr>
          <td>2</td>
          <td>Students attendance and his/her regularity</td>
          <td><label for="attendance12"><?php echo $row['Attendance']; ?> </td>
      </tr>
      <tr>
          <td>3</td>
          <td>Semester Result of the Earlier Semester</td>
          <td><label for="result12"></label><?php echo $row['Result']; ?></td>
      </tr>
      <tr>
          <td>4</td>
          <td>Participation in various activities(Technical/Sports/Cultural/Co-carricular)</td>
          <td><label for="carricular12"></label><?php echo $row['Carricular']; ?> </td>
      </tr>
      <tr>
          <td>5</td>
          <td>Communication and Professional Skills</td>
          <td><label for="candp12"></label> <?php echo $row['CandP_Skills']; ?> </td>
      </tr>
      <tr>
          <td>6</td>
          <td>General Behaviour and Discipline</td>
          <td><label for="behaviour12"></label> <?php echo $row['Behaviour']; ?></td>
      </tr>
      <tr>
          <td>7</td>
          <td>Any Other Specify</td>
          <td><label for="other12"></label> <?php echo $row['Other']; ?></td>
      </tr>
            </tbody>
          </table>
          <a href="sems_report12_PDF.php?sprn=<?php echo $selectedPRN; ?>" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
                                  class="fas fa-download fa-sm text-white-50"></i> Generate Report</a>
                      </div>
          <br>
          <div class="button text-center">
                      <a href="sem_report12.php?sprn=<?php echo $selectedPRN; ?>" class="btn btn-primary btn-icon-split">
