            ...
            <div id="content">
                ...
                <div class="button text-center">
                    <button type="submit" name="submit" class="btn btn-success btn-icon-split">
                        <a href="tmeeting.php" class="btn btn-success btn-icon-split">
                            <span class="icon text-white-50">
                                <i class="fas fa-check"></i>
                            </span>
                            <span class="text">Schedule Meeting</span></a></button>
                    <button type="submit" name="submit" class="btn btn-success btn-icon-split">
                        <a href="hmeeting.php" class="btn btn-primary btn-icon-split">
                            <span class="icon text-white-50">
                                <i class="fas fa-eye"></i>
                            </span>
                            <span class="text">Meeting History</span></a></button>
                ...
                <div class="container-fluid">
                    <div class="card shadow mb-4">
                        <div class="card-header py-3" style="background-color: #4e73df;">
                            <h6 class="m-0 font-weight-bold" style="color: #fff;">Scheduled Meetings</h6>
                        </div>
                                    ...
                                    <table class="table table border" id="dataTable" width="100%" cellspacing="0">
                                        <tbody>
                                            ...
                                            if (isset($_POST['updateMeeting'])) {
                                                $meetingPRN = $_POST['meetingPRN'];
                                                $meetingDate = $_POST['meetingDate'];
                                                $meetingStatus = $_POST['meetingStatus'];
                                                $meetingResponse = $_POST['meetingResponse'];

                                                // Perform the update query to update the Status and Response columns
                                                $updateQuery = "UPDATE `meetings` SET `Status`='$meetingStatus', `Response`='$meetingResponse' WHERE `PRN`='$meetingPRN' AND `Date`='$meetingDate'";
                                                mysqli_query($conn, $updateQuery);

                                            }
                                            ?>
                                            <?php
                                            if (isset($_POST['deletePRN'])) {
                                                $selectedPRN = $_POST['deletePRN'];

                                                // Perform the deletion query
                                                $deleteQuery = "DELETE FROM `meetings` WHERE `PRN`='$selectedPRN'";
                                                mysqli_query($conn, $deleteQuery);

                                            }
                                            ?>
                                            <?php
                                            $query = mysqli_query($conn, "SELECT * FROM `meetings` WHERE `tid`='$TID' AND `Response` IS NULL OR `Response` = ''");
                                            while ($row = mysqli_fetch_assoc($query)) {
                                                ...
                                                echo "<td>";
                                                echo "<form action='tmeeting_table.php' method='POST' style='display:inline-block'>";
                                                echo "<input type='hidden' name='meetingPRN' value='" . $row['PRN'] . "'>";
                                                echo "<input type='hidden' name='meetingDate' value='" . $row['Date'] . "'>";
                                                echo "<label><input type='radio' name='meetingStatus' value='present' checked>Present</label>&nbsp;";
                                                echo "<label><input type='radio' name='meetingStatus' value='absent'>Absent</label>";
                                                echo "</td>";
                                                echo "<td>";
                                                echo "<input type='text' name='meetingResponse'>";
                                                echo "</td>";
                                                echo "<td>";
                                                echo "<button type='submit' class='btn btn-primary' name='updateMeeting'>Update</button>";
                                                echo "</form>";
                                                echo "</td>";
                                                echo "<td>";
                                                echo "<form action='tmeeting_table.php' method='POST' style='display:inline-block'>";
                                                echo "<input type='hidden' name='deletePRN' value='" . $row['PRN'] . "'>";
